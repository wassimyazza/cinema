...
@section('title', 'Modifier la séance - Administration CinéHall')
...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <div class="lg:flex lg:items-center lg:justify-between mb-6">
            <div class="flex-1 min-w-0">
                <h1 class="text-2xl font-semibold text-gray-900">Modifier la séance</h1>
                <p class="mt-1 text-sm text-gray-500">
                    Modifier les informations de la séance de {{ $showtime->movie->title }} du {{ \Carbon\Carbon::parse($showtime->start_time)->format('d/m/Y H:i') }}
                </p>
            </div>
            <div class="mt-5 flex lg:mt-0 lg:ml-4">
                ...
                <a href="{{ route('showtimes.show', $showtime->id) }}" class="ml-3 inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" target="_blank">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <form action="{{ route('admin.showtimes.update', $showtime->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <div class="mt-1">
                                <input type="date" name="start_date" id="start_date" 
                                       value="{{ old('start_date', \Carbon\Carbon::parse($showtime->start_time)->format('Y-m-d')) }}" 
                                       required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                            </div>
                            @error('start_date')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <div class="mt-1">
                                <input type="time" name="start_time" id="start_time" 
                                       value="{{ old('start_time', \Carbon\Carbon::parse($showtime->start_time)->format('H:i')) }}" 
                                       required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                            </div>
                            @error('start_time')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <div class="mt-1">
                                <select id="language" name="language" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                    <option value="Français" {{ old('language', $showtime->language) == 'Français' ? 'selected' : '' }}>Français</option>
                                    <option value="Anglais" {{ old('language', $showtime->language) == 'Anglais' ? 'selected' : '' }}>Anglais (VOST)</option>
                                    <option value="Espagnol" {{ old('language', $showtime->language) == 'Espagnol' ? 'selected' : '' }}>Espagnol (VOST)</option>
                                    <option value="Version originale" {{ old('language', $showtime->language) == 'Version originale' ? 'selected' : '' }}>Version originale (VOST)</option>
                                </select>
                            </div>
                            @error('language')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <div class="mt-1">
                                <select id="type" name="type" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                    <option value="Normal" {{ old('type', $showtime->type) == 'Normal' ? 'selected' : '' }}>Normal</option>
                                    <option value="VIP" {{ old('type', $showtime->type) == 'VIP' ? 'selected' : '' }}>VIP</option>
                                </select>
                            </div>
                            @error('type')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="border-t border-gray-200 px-4 py-5 sm:p-6">
                    <div id="availability-info" class="bg-gray-50 p-4 rounded-md">
                        <p class="text-sm text-gray-500">
                            Si vous modifiez la date, l'heure ou la salle, veuillez vérifier la disponibilité.
                        </p>
                    </div>
                    
                    <div id="current-info" class="mt-4 bg-blue-50 p-4 rounded-md">
                        <div class="flex">
                            <div class="flex-shrink-0">
                                <i class="fas fa-info-circle text-blue-400"></i>
                            </div>
                            <div class="ml-3">
                                <h3 class="text-sm font-medium text-blue-800">Information sur la séance actuelle</h3>
                                <div class="mt-2 text-sm text-blue-700">
                                    <p>Cette séance a actuellement {{ $showtime->reservations->count() }} réservations, dont {{ $showtime->reservations->where('status', 'paid')->count() }} payées.</p>
                                    <p class="mt-1">Modifier l'horaire de la séance n'affectera pas les réservations existantes.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-green-700">
                                    <p>La salle est disponible à l'horaire sélectionné. Vous pouvez modifier cette séance.</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Showtime ID for excluding current showtime from conflict check
    ...
    async function checkAvailability() {
        ...
        try {
            ...
            //         end_time: endTime,
            //         exclude_showtime_id: showtimeId
            //     })
            // });
    ...
    async function simulateAvailabilityCheck(theaterId, startTime, endTime) {
        ...
        // Original theater and movie ID to check if we're changing the theater
        const originalTheaterId = "{{ $showtime->theater_id }}";
        
        // If we're not changing the theater and time, it's automatically available (it's the current showtime)
        if (theaterId === originalTheaterId &&
            startDateInput.value === "{{ \Carbon\Carbon::parse($showtime->start_time)->format('Y-m-d') }}" &&
            startTimeInput.value === "{{ \Carbon\Carbon::parse($showtime->start_time)->format('H:i') }}") {
            return {
                available: true,
                conflicts: []
            };
